...
def setup_ide_configuration(
    ...
    if not PoetryDetector.is_in_poetry_project(project_path):
        if verbose:
            console.print(
                "[yellow]Warning: No pyproject.toml with Poetry configuration found[/yellow]"
            )
    ...
    # Get Python version
    try:
        python_version = PoetryDetector.get_python_version(interpreter_path)
        if verbose:
            console.print(f"[dim]Python version: {python_version}[/dim]")
    except Exception as e:
        if verbose:
            console.print(
                f"[dim]Warning: Could not determine Python version: {e}[/dim]"
            )
        python_version = "3.x"  # fallback
    ...
    if not ProjectDetector.validate_project_structure(project_path):
        if verbose:
            console.print(
                "[yellow]Warning: Project structure validation failed[/yellow]"
            )
    ...
    if not ProjectDetector.is_python_project(idea_path):
        if verbose:
            console.print(
                "[yellow]Warning: Project doesn't appear to be configured for Python[/yellow]"
            )

    # Create Python SDK name
    python_sdk_name = f"Python {python_version} {project_name}"
    if verbose:
        console.print(f"[dim]Python SDK name: {python_sdk_name}[/dim]")
    ...
    needs_update = force or current_interpreter != python_sdk_name
    ...
    if not needs_update:
        ...
        return SetupResult(
            ...
            python_sdk_name=python_sdk_name,
    ...
    return SetupResult(
        ...
        python_sdk_name=python_sdk_name,
        config_file=misc_xml_path,
        was_updated=True,  # Indicate that an update was needed/performed
        previous_interpreter=current_interpreter,
    )
