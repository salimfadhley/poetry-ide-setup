...
class TestPoetryDetector:
    ...
    def test_get_interpreter_path_success_env_info(self, mock_run: MagicMock, mock_interpreter_path: Path) -> None:
    ...
    @patch('subprocess.run')
    def test_get_interpreter_path_fallback_which_python(self, mock_run: MagicMock, mock_interpreter_path: Path) -> None:
    ...
    @patch('subprocess.run')
    def test_get_interpreter_path_no_interpreter(self, mock_run: MagicMock, temp_dir: Path) -> None:
        ...
        with pytest.raises(InterpreterNotFoundError, match="no Python interpreter could be detected"):
            PoetryDetector.get_interpreter_path()

    @patch('subprocess.run')
    def test_get_environment_info_success(self, mock_run: MagicMock, mock_interpreter_path: Path) -> None:
    ...
    @patch('subprocess.run')
    def test_get_environment_info_fallback_name(self, mock_run: MagicMock, mock_interpreter_path: Path) -> None:
    ...
    def test_is_in_poetry_project_current_directory(self, mock_poetry_project: Path) -> None:
